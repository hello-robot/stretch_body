...
from stretch_body.trajectory import Waypoint, generate_cubic_spline_segment
...
class TrajectoryScope:
    def __init__(self, joint_name, preloaded_key, velocity_ctrl=False):
        ...
        self.is_stepper = isinstance(self.traj_man, StepperTrajectoryManager)

        if args.velocity_ctrl and self.is_stepper:
            raise RuntimeError('Cannot use velocity control with stepper joints!')
    ...
    def _update(self, e):
        ...
        for (i, f) in zip(waypoints, waypoints[1:]):
            seg = generate_cubic_spline_segment(i, f)
            segx = np.arange(i.t, f.t, 0.05)
            for t in segx:
                splinex.append(t)
                spliney.append(seg.evaluate_at(t).p)
    ...
    def _execute(self, e):
        if not self.executing:
            ...
            if self.is_stepper:
                self.traj_man.start_trajectory(threaded=False)
            else:
                self.traj_man.start_trajectory(position_ctrl=not self.velocity_ctrl, threaded=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--velocity_ctrl', help='Use velocity control to follow trajectory', action='store_true')
    parser.add_argument('-p', '--preloaded_traj', help='Load one of three predefined trajectories',
                        choices=['1', '2', '3'], default='1')
    args = parser.parse_args()

    scope = TrajectoryScope(args.joint, args.preloaded_traj, args.velocity_ctrl)
