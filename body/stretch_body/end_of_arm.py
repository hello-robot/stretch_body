...
class EndOfArm(DynamixelXChain):
    ...
    def get_joint_configuration(self,brake_joints={}):
        ...
        if gripper_joint:
            gripper = self.get_joint(gripper_joint)

            # Custom grippers, like aloha_gripper, may not define SG3-style
            # gripper_conversion geometry. Skip finger joints in that case.
            if 'gripper_conversion' in gripper.status:
                dx = 0
                if brake_joints:
                    for j in brake_joints:
                        if 'gripper' in j:
                            dx = self.params['collision_mgmt']['k_brake_distance'][j]
                finger_angle = gripper.status['gripper_conversion']['finger_rad'] + dx
