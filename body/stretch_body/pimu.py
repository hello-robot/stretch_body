...
class IMUBase(Device):
    ...
    def __init__(self):
        ...
        self.status = {'ax': 0, 'ay': 0, 'az': 0, 'gx': 0, 'gy': 0, 'gz': 0, 'mx': 0, 'my': 0, 'mz': 0, 'roll': 0,
...
class PimuBase(Device):
    ...
    def __init__(self, event_reset=False, usb=None):
        ...
        self.status = {'voltage': 0, 'current': 0, 'temp': 0, 'cpu_temp': 0, 'cliff_range': [0, 0, 0, 0], 'frame_id': 0,
                       'timestamp': 0, 'at_cliff': [False, False, False, False], 'runstop_event': False,
                       'bump_event_cnt': 0,
                       'cliff_event': False, 'fan_on': False, 'buzzer_on': False, 'low_voltage_alert': False,
                       'high_current_alert': False, 'over_tilt_alert': False,
                       'charger_connected': False, 'boot_detected': False, 'imu': self.imu.status, 'debug': 0,
                       'state': 0, 'trace_on': 0,
        ...
        if self.ts_last_motor_sync is not None:
            ...
            if rate > self.params['max_sync_rate_hz']:
                ...
                if self.ts_last_motor_sync_warn is None or t - self.ts_last_motor_sync_warn > 5.0:
                    print(
                        'Warning: Rate of calls to Pimu:trigger_motor_sync rate of %f above maximum frequency of %.2f Hz. Motor commands dropped: %d' % (
                        rate, self.params['max_sync_rate_hz'], self.status['motor_sync_drop']))
    ...
    def unpack_board_info(self, s):
        ...
        if len(self.board_info[
                   'board_variant']) == 6:  # New format of Pimu.x Older format of Pimu.BoardName.Vx' If older format,default to 0
    ...
    def pack_config(self, s, sidx):
        for i in range(4):
            ...
            sidx += 4
        pack_float_t(s, sidx, self.config['cliff_thresh']);
        sidx += 4
        pack_float_t(s, sidx, self.config['cliff_LPF']);
        sidx += 4
        ...
        sidx += 4
        pack_uint8_t(s, sidx, self.config['stop_at_cliff']);
        sidx += 1
        pack_uint8_t(s, sidx, self.config['stop_at_runstop']);
        sidx += 1
        pack_uint8_t(s, sidx, self.config['stop_at_tilt']);
        sidx += 1
        pack_uint8_t(s, sidx, self.config['stop_at_low_voltage']);
        sidx += 1
        pack_uint8_t(s, sidx, self.config['stop_at_high_current']);
        sidx += 1
        for i in range(3):
        ...
        sidx += 4
        pack_float_t(s, sidx, self.config['low_voltage_alert']);
        sidx += 4
...
class Pimu_Protocol_P0(PimuBase):
    def unpack_status(self, s, unpack_to=None):
        ...
        unpack_to['timestamp'] = self.timestamp.set(unpack_uint32_t(s[sidx:]));
        sidx += 4
...
class Pimu_Protocol_P6(PimuBase):
    def unpack_status(self, s, unpack_to=None):  # P6
        ...
        unpack_to['over_tilt_type'] = self.get_tilt_type(unpack_uint8_t(s[sidx:]));
        sidx += 1
        return sidx

    def actuator_control(self, motor_type, enable):
        mt = [None, 'lift', 'left-wheel', 'right-wheel', 'center-wheel', 'arm', 'eoa']
        ...
        payload = self.transport.get_empty_payload()
        if enable:
            payload[0] = self.RPC_ACTUATOR_ENABLE
            payload[1:] = arr.array('B', [motor_type])
            self.transport.do_push_rpc_sync(payload, self.rpc_actuator_enable_reply)
        else:
            payload[0] = self.RPC_ACTUATOR_DISABLE
            payload[1:] = arr.array('B', [motor_type])
            self.transport.do_push_rpc_sync(payload, self.rpc_actuator_disable_reply)

    def rpc_actuator_enable_reply(self, reply):
        if reply[0] != self.RPC_REPLY_ACTUATOR_ENABLE:
            print('Error RPC_REPLY_ACTUATOR_CONTROL', reply[0])

    def rpc_actuator_disable_reply(self, reply):
...
class Pimu(PimuBase):
    ...
    def __init__(self, event_reset=False, usb=None):
        ...
        self.supported_protocols = {'p0': (Pimu_Protocol_P0,),
                                    ...
                                    'p4': (Pimu_Protocol_P4, Pimu_Protocol_P3, Pimu_Protocol_P2, Pimu_Protocol_P1,
                                           Pimu_Protocol_P0,),
                                    'p5': (Pimu_Protocol_P5, Pimu_Protocol_P4, Pimu_Protocol_P3, Pimu_Protocol_P2,
                                           Pimu_Protocol_P1, Pimu_Protocol_P0,),
                                    'p6': (Pimu_Protocol_P6, Pimu_Protocol_P5, Pimu_Protocol_P4, Pimu_Protocol_P3,
                                           Pimu_Protocol_P2, Pimu_Protocol_P1, Pimu_Protocol_P0,)
                                    }
